    ...
    var directive = function () {
        return {
            ...
            scope: {
                ...
                minLength:'@', //minima longitud del contenido del tag
                maxLength:'@', //maxima longitud del contenido del tag
                validation:'<?' //validacion pattern para contenido de tags
            },
...
angular.module('angular-tag/templates', [])
    .run(['$templateCache', function($templateCache) {
        $templateCache.put('angular-tag/templates/input.html',`<div id="main-tag">\n
                                                                <div class="tag-container">\n
                                                                  <ul ng-class="[\'tag\',{focus:isFocus},theme ]" ng-click="isFocus=true" ng-keydown="direction_keys($event)">\n
                                                                    <li ng-repeat="select in selected" ng-click="(moveToTag($event,$index)) "  data-drop="true" data-drag="true" ng-model="selected" jqyoui-droppable="{index: {{$index}}}" jqyoui-draggable="{index: {{$index}}, insertInline: true, direction:\'jqyouiDirection\'}" data-jqyoui-options="{revert: \'invalid\'}" data-direction="{{item.jqyouiDirection}}">\n      {{select[displayField]}}
                                                                      <a href="javascript:void(0)" ng-click="remove(select)">&times;</a>\n
                                                                       </li>\n<li>\n
                                                                         <input ng-pattern="validation" ng-minlength="{{minLength}}" ng-maxlength="{{maxLength}}" name="{{name}}" ng-model="input" ng-keyup="on_input_keyup($event)" ng-keydown="on_input_keydown($event)" ng-focus="isFocus=true" ng-blur="onblur();" focus-me="isFocus"\n type="input" placeholder="{{placeholder}}" ng-required="required && (!selected.length || selected.length==0)" >\n
                                                                       </li>\n
                                                                  </ul>\n
                                                                </div>\n\n
